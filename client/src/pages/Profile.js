    ...
    const displayItems = myItems.slice( visitedPages, visitedPages + itemsPerPage ).map((value, key)=>{
            return <div className='item' onClick={()=>{navigate(`/item/${value.id}`)}}> 
                    ...
                    <div className='footer'>
                        ...
                        <div style={{ color: 'white' }}> {value.currently} € &nbsp;&nbsp;</div>
                    </div>
                    </div>
            ;
    });
  ...
  return (
    <>
        ...
        { Object.keys(coordinates).length > 0 &&
            ...
            { !userInfo.approved &&
                            <>
                            <Typography style={{fontFamily:'Roboto Slab',color:'navy'}}>{`${userInfo.username} hasn't been approved yet.`}</Typography>
        ...
        { myItems.length!==0 && (
            <>
                  ...
                  <Typography style={{backgroundColor:'#00203FFF',fontFamily:'Roboto Slab',color:'white',borderRadius:'10px',padding:'10px'}} variant='h4' >{`${userInfo.username}'s Auctions`}</Typography>
