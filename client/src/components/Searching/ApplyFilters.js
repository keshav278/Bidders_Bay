...
export default function ApplyFilters(props) {
  ...
  return (
    ...
    >
      <Box sx={style}>
        <Typography sx={{fontFamily:'Roboto Slab'}} id="modal-modal-title" variant="h5" component="h2">
        Select One or Multiple Filters
        </Typography>
        <Typography variant="h6" id="modal-modal-description" sx={{ mt: 2,fontFamily:'Roboto Slab' }}>
          Price Range <PriceRange max={props.max} setPriceRange={setInterPriceRange} />
        </Typography>  
        <Typography variant="h6" id="modal-modal-description" sx={{ mt: 1,fontFamily:'Roboto Slab' }}>
          Country
        </Typography>
        <CountryDropdown 
              className='buttonitoSelect'
              style={{ width: "230px", fontFamily:'Roboto Slab' }}
              value={mycountry}
